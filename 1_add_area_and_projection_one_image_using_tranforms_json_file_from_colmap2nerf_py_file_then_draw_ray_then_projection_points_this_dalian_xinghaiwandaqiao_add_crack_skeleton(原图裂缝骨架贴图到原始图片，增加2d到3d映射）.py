...
markers_corners_world_coords, _, _ = pixel_to_world(selected_area_np_qici, 0.5),  # depth=0.5是在虚拟位姿的矩形框展示
...
result_image_magma, mask_zero_same_size_rgb_pil = merge_imgs_and_masks(org_img_dir, mask_dir, color_list=class_colors_list)
...
# mesh + points + 相机中心点
    ...
    # 保存可视化结果
    o3d.io.write_point_cloud(os.path.join(f'{save_output_mesh_ray_dir}', f'scene_added_area_skeleton_org_size_mask_img_id={img_name}.ply'), pcd)
    print(f'已保存可视化结果: {save_output_mesh_ray_dir}/scene_added_area_skeleton_org_size_mask_img_id={img_name}.ply')
    ...
    o3d.io.write_triangle_mesh(os.path.join(f'{save_output_mesh_ray_dir}', f'ray_hit_mesh_skeleton_org_size_mask_img_id={img_name}.ply'), mesh)
    print(f'已保存可视化结果: {save_output_mesh_ray_dir}/ray_hit_mesh_skeleton_org_size_mask_img_id={img_name}.ply')
