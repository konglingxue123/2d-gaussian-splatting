...
################################### 4. 损伤mask与相机中心点的连线 #################################
# 生成网格点矩阵
y_range_defects = slice(add_xy1[1], add_xy2[1])  # y 对应行索引
print('y_range_defects, type(y_range_defects):', y_range_defects, type(y_range_defects))
x_range_defects = slice(add_xy1[0], add_xy2[0])  # x 对应列索引
...
for id, defect_point in enumerate(defects_area_np):
    defects_area_mask_world_list.append(world_coords[(defect_point[1]-1)*im.size[0]+defect_point[0]])
...
# mesh + points + 相机中心点
    ...
    added_colors = np.array([[1, 0, 0]]*(np.array(hit_points).shape[0]))
    ...
    # 保存可视化结果
    o3d.io.write_point_cloud(os.path.join(f'{save_output_mesh_ray_dir}', 'scene_added_area.ply'), pcd)
    print(f'已保存可视化结果: {save_output_mesh_ray_dir}/scene_added_area.ply')
    ...
    o3d.io.write_triangle_mesh(os.path.join(f'{save_output_mesh_ray_dir}', 'ray_hit_mesh.ply'), mesh)
    print(f'已保存可视化结果: {save_output_mesh_ray_dir}/ray_hit_mesh.ply')
