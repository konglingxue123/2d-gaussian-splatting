...
# render_mesh_dir = '/home/ubunto/Project/konglx/pcd/2dgs/2d-gaussian-splatting-main/output/dalian_xinghaiwandaqiao_rgba_read_txt_camera_coords_selected_DBSCAN'
render_mesh_dir = '/home/ubunto/Project/konglx/pcd/2dgs/2d-gaussian-splatting-main/output/dalian_xinghaiwandaqiao_video_input_rgba_txt_file_camera_crop_OPTICS_lambda_dist=100'
save_output_mesh_ray_dir = f'{render_mesh_dir}/based_on_2d_npy_file_edge_all'
os.makedirs(save_output_mesh_ray_dir, exist_ok=True)
org_img_dir = f'/home/ubunto/Project/konglx/pcd/2dgs/2d-gaussian-splatting-main/datasets/dalian_xinghaiwandaqiao_video_input_rgb/outputs/labelme_outputs_new/JPEGImages'
...
if num_classes == 1:
    ...
    class_colors_list = [[128, 0, 0], [0, 128, 0], [0, 128, 128]]
else:
...
mask_dir = '/home/ubunto/Project/konglx/pcd/2dgs/2d-gaussian-splatting-main/datasets/dalian_xinghaiwandaqiao_video_input_rgb/outputs/labelme_outputs_new/SegmentationClass'
mask_member_dir = '/home/ubunto/Project/konglx/pcd/2dgs/2d-gaussian-splatting-main/datasets/dalian_xinghaiwandaqiao_video_input_rgba/outputs/SegmentationClass'
...
for id, frame in enumerate(frames_data):
    for img_name in img_name_list:
        if img_name in frame['file_path']:
            ...
            mask_member = Image.open(os.path.join(mask_member_dir, img_name+'.png'))
            mask_member_np = np.array(mask_member)
            mask_member_np_flatten = mask_member_np.flatten()
            
            # 图像的rgb点
            # img_np = np.asarray(im) / 255.0
            img_pil_dir = os.path.join(org_img_dir, img_name+'.jpg')
            img_pil_new = Image.open(img_pil_dir)
            result_image_magma, mask_zero_same_size_rgb_pil = merge_imgs_pil_and_masks_np(img_pil_new, img_name, crack_mask_magma_pil_l_np, color_list=class_colors_list)
            result_image_magma_pil = Image.fromarray(result_image_magma)
            img_np = result_image_magma / 255.0
            # img_np.shape, img_np.shape[0]*img_np.shape[1]
            img_np_reshape = img_np.reshape(-1, 3)
            # img_np_reshape = img_np_reshape[mask_member_np_flatten == 1]
            world_colors_list.append(img_np_reshape)
            ...
            # 组合为二维坐标点，并调整形状为 [30000, 2]
            selected_area_np = np.column_stack((x.ravel(), y.ravel()))
            # selected_area_np = selected_area_np[mask_member_np_flatten == 1]
            selected_area_np_qici = np.hstack((selected_area_np, np.ones((selected_area_np.shape[0], 1))))
            print(selected_area_np_qici.shape)
            # 内参重复10000次，shape为[10000, 4, 4]
            intrinsics_area = np.tile(intrinsics, (selected_area_np_qici.shape[0], 1, 1))
            ...
            world_coords, _, _ = pixel_to_world(selected_area_np_qici, 0.5,  # depth=0.5是在虚拟位姿的矩形框展示
                                        intrinsics_area[:,:3, :3], 
                                        extrinsics_area[:, :3, :3], 
                                        extrinsics_area[:,:3, 3])
...
world_coords_np = np.vstack(world_coords_list)
print('world_coords_np.shape', world_coords_np.shape)

world_colors_np = np.vstack(world_colors_list)
print('world_colors_np.shape', world_colors_np.shape)

print('用于选择相机位子的id_list:', id_list)
...
# 展示所有
for img_selected_name in img_name_list:
    for frame in frames_data:
        if img_selected_name in frame['file_path']:
            R = np.array(frame['R'])
            t = np.array(frame['t'])
...
# 损伤类别
num_classes = 2
defects_area_mask_world_list_all = []
chosen_camera_center_coords_list_all = []
crack_mask_magma_np_color_not_zero_list_all = []

each_img_point_num_list = []
directions_list = []
endpoints_list = []

# chosen_camera_center_coords_list = []
t_hits_list = []
ray_hit_mesh_list = []
...
for id_num,img_name in enumerate(img_name_list):
    crack_mask_magma_dir = f'{mask_dir}/{img_name}.png'
    # crack_mask_magma_dir = '/home/ubunto/Project/konglx/pcd/2dgs/2d-gaussian-splatting-main/output/projection/CFD_044.jpg'
    crack_mask_magma_pil_l = Image.open(crack_mask_magma_dir)
    crack_mask_magma_pil_l_np = np.zeros((crack_mask_magma_pil_l.size[1], crack_mask_magma_pil_l.size[0])).astype(np.uint8)
    ...
    ############### 读取npy文件，并将其转换为图片 #############
    cls_num_edge_pair_dict_npy_dir = f'/home/ubunto/Project/konglx/pcd/2dgs/2d-gaussian-splatting-main/output/dalian_xinghaiwandaqiao_video_input_rgba_txt_file_camera_crop_OPTICS_lambda_dist=100/img_point_width_new_segmentation/edge_skel_edge_pair/cls_num_edge_pair_dict/{img_name}.npy'
    cls_num_edge_to_calculate_mid_axis_dict_npy_dir = f'/home/ubunto/Project/konglx/pcd/2dgs/2d-gaussian-splatting-main/output/dalian_xinghaiwandaqiao_video_input_rgba_txt_file_camera_crop_OPTICS_lambda_dist=100/img_point_width_new_segmentation/edge_skel_edge_pair/cls_num_edge_to_calculate_mid_axis_dict/{img_name}.npy' 
    # cls_num_skel_calculate_mid_axis_dict_npy_dir = '/home/ubunto/Project/konglx/pcd/2dgs/2d-gaussian-splatting-main/output/dalian_xinghaiwandaqiao_video_input_rgba_txt_file_camera_crop_OPTICS_lambda_dist=100/img_point_width_new_segmentation/edge_skel_edge_pair/cls_num_skel_calculate_mid_axis_dict_0032.npy' 
    ...
    if img_name == '0032':
        ...
        x_defects_in_img = x_defects_in_img
        y_defects_in_img = y_defects_in_img
    ...
    print('############ defects_area_np ############', defects_area_np.shape)
    defects_area_np_qici = np.hstack((defects_area_np, np.ones((defects_area_np.shape[0], 1))))
    print('############ defects_area_np_qici ############', defects_area_np_qici.shape)


    mask_zero_same_size_rgb_np = np.array(mask_zero_same_size_rgb_pil)#[:,:,::-1]  # 转换为RGB格式
    print('maskzero_rgb_np.shape:', mask_zero_same_size_rgb_np.shape, np.unique(mask_zero_same_size_rgb_np))
    ...
    for id, defect_point in enumerate(defects_area_np):
        defects_area_mask_world_list.append(world_coords_list[id_num][defect_point[1] + (defect_point[0]-1)*w])
    ...
    each_img_point_num_list.append(defects_area_mask_world.shape[0])
    ...
    print('############ defects_area_mask_world ############', defects_area_mask_world, defects_area_mask_world.shape)
    # print('id_list, id_num:', id_list, id_num)
    chosen_camera_center_coords = camera_center_coords[id_list[id_num]].reshape(-1, 3)
    ...
    direction_each_img = defects_area_mask_world - chosen_camera_center_coords
    direction_each_img /= np.linalg.norm(direction_each_img, axis=1, keepdims=True)
    endpoint_each_img = defects_area_mask_world + direction_each_img * line_length
    
    
    defects_area_mask_world_list_all.append(defects_area_mask_world)
    chosen_camera_center_coords_list_all.append(chosen_camera_center_coords)
    crack_mask_magma_np_color_not_zero_list_all.append(crack_mask_magma_np_color_not_zero_list)

    directions_list.append(direction_each_img)
    endpoints_list.append(endpoint_each_img)

# defects_area_mask_world_all_np = np.vstack(defects_area_mask_world_list_all)
# chosen_camera_center_coords_all_np = np.vstack(chosen_camera_center_coords_list_all)
# crack_mask_magma_np_color_not_zero_list_all_np   = np.vstack(crack_mask_magma_np_color_not_zero_list_all)
# print('############ defects_area_mask_world_all_np ############',  defects_area_mask_world_all_np.shape)
# print('############ chosen_camera_center_coords_all_np ############',  chosen_camera_center_coords_all_np.shape)
# print('############ crack_mask_magma_np_color_not_zero_list_all_np ############',  crack_mask_magma_np_color_not_zero_list_all_np.shape)
# 计算方向向量并归一化
# directions_list = []
# endpoints_list = []
# for i, each_num_points in enumerate(each_img_point_num_list):
#     direction = defects_area_mask_world_all_np[:each_num_points] - chosen_camera_center_coords_all_np[i]
#     direction /= np.linalg.norm(direction, axis=1, keepdims=True)
#     endpoint_each_img = defects_area_mask_world_all_np[:each_num_points] + direction * line_length
    
#     directions_list.append(direction)
#     endpoints_list.append(endpoint_each_img)
    
# directions = np.vstack(directions_list)
# print('############ derections ############', directions, directions.shape)
# endpoints = np.vstack(endpoints_list)
# print('############ endpoints ############', endpoints, endpoints.shape)
# directions = defects_area_mask_world_all_np - chosen_camera_center_coords_all_np
# directions /= np.linalg.norm(directions, axis=1, keepdims=True)
# 延长后的终点
# endpoints = defects_area_mask_world_all_np + directions * line_length

    # 连接点云与相机中心点(第一行为相机中心点，其他为损伤点)
    concate_camera_center_to_defects_points = np.vstack([chosen_camera_center_coords, endpoint_each_img])
...
# 5.1 转换为 TensorMesh 并添加到 RaycastingScene
    ...
    chosen_camera_center_coords_same_shape_with_rays = np.tile(chosen_camera_center_coords, (direction_each_img.shape[0], 1))
    rays = np.concatenate([chosen_camera_center_coords_same_shape_with_rays, direction_each_img], axis=1)
    ...
    # chosen_camera_center_coords_list.append(chosen_camera_center_coords)
    t_hits_list.append(t_hits)
    ray_hit_mesh_list.append(ray_hit_mesh)
    
    hit_points_each_img = []
    hit_points_colors_each_img = []
    print('direction_each_img', direction_each_img, direction_each_img.shape)
    print('t_hits', t_hits, t_hits.shape)
    print(f'计算第{id_num+1}张图片的射线投射点...')
    for i_direction in range(len(direction_each_img)):
        if np.isfinite(t_hits[i_direction]):
            # print('chosen_camera_center_coords_list_all:', chosen_camera_center_coords_list_all)
            
            hit_point = chosen_camera_center_coords_list_all[id_num][0] + direction_each_img[i_direction] * t_hits[i_direction]
            # print('chosen_camera_center_coords_list_all[i]:',chosen_camera_center_coords_list_all[id_num], type(chosen_camera_center_coords_list_all[id_num]), type(chosen_camera_center_coords_list_all[id_num][0]))
            # print('chosen_camera_center_coords:', chosen_camera_center_coords, type(chosen_camera_center_coords), type(chosen_camera_center_coords[0]))
            
            hit_points_each_img.append(hit_point)
            hit_points_all_list.append(np.vstack(hit_points_each_img))
            
            hit_points_colors_each_img.append(crack_mask_magma_np_color_not_zero_list[i_direction])
            hit_colors_all_list.append(np.vstack(hit_points_colors_each_img))
            # 绘制射线
            line = o3d.geometry.LineSet(
                points=o3d.utility.Vector3dVector([chosen_camera_center_coords_list_all[id_num][0], hit_point]),
            ...
            points.colors = o3d.utility.Vector3dVector([crack_mask_magma_np_color_not_zero_list[i_direction]])
            ...
            try:
                ...
                for i, cls_item in enumerate(cls_list):
                    ...
                    for points_id in range(len(cls_item)):
                        ...
                        cls_hitpoints_dict[cls][i].append(hit_points_each_img[points_id])
                        cls_hitpoints_dict[cls][i+1].append(hit_points_each_img[points_id+1])
                        # 3维点的距离
                        dist_each_3d_point = distance(hit_points_each_img[points_id], hit_points_each_img[points_id+1])
                        cls_dist_each_3d_point_dict[cls][i].append(dist_each_3d_point)
                        
                        # 计算中心点
                        center_point = (hit_points_each_img[points_id] + hit_points_each_img[points_id+1]) / 2
                ...
                continue
        # 存储hit_points
        pcd_color_dict = {'all_points_list': hit_points_each_img,
...
# hit_points = []
# hit_points_colors = []
# for each_img_direction in directions:
#     for i in range(len(each_img_direction)):
#         if np.isfinite(t_hits[each_img_direction][i]):
#             hit_point = chosen_camera_center_coords_list_all[i] + each_img_direction[i] * t_hits[i]
#             hit_points.append(hit_point)
#             hit_points_colors.append(crack_mask_magma_np_color_not_zero_list_all[i])
...
hit_points = np.vstack(hit_points_all_list)
scene_points = np.vstack((scene_points, world_coords_np, camera_center_points, hit_points))
pcd.points = o3d.utility.Vector3dVector(scene_points)
...
added_colors = np.array(np.vstack(hit_colors_all_list))
# added_colors = img_np_reshape
scene_colors = np.vstack((scene_colors, world_colors_np, camera_center_coords_colors, added_colors))
pcd.colors = o3d.utility.Vector3dVector(scene_colors)
